...
- Though using Eloquent Models is allowed in temporary migrations, it is highly recommended that they be used very sparingly, if at all. If you choose to use them you MUST consider and possibly mitigate side effects of their use such as observers, global scopes on the model, etc. To avoid dealing with this, it is acceptable to choose to always use the DB facade instead of Models.
