...
- In [app-modules/ai/src/Policies/AiAssistantPolicy.php](../app-modules/ai/src/Policies/AiAssistantPolicy.php): remove the `AiAssistantDtoRenameFeature::active()` ternaries in `viewAny()` and `create()` — keep only `Feature::EmployeeAdvisors->getGateName()` and `app(LicenseSettings::class)->data->limits->employeeAdvisorsCount`.
