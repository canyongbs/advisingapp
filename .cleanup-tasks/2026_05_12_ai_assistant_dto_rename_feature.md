...
- In [app/Http/Requests/Tenants/CreateTenantRequest.php](../app/Http/Requests/Tenants/CreateTenantRequest.php): remove the `AiAssistantDtoRenameFeature::active()` conditional — keep only the new validation rules (`limits.employeeAdvisors`, `limits.customerAdvisorsCount`, `addons.employeeAdvisors`, `addons.customerAdvisors`).
- In [app/Http/Requests/Tenants/SyncTenantRequest.php](../app/Http/Requests/Tenants/SyncTenantRequest.php): remove the `AiAssistantDtoRenameFeature::active()` conditional — keep only the new validation rules (`limits.employeeAdvisors`, `limits.customerAdvisorsCount`, `addons.employeeAdvisors`, `addons.customerAdvisors`).
