...
Route::middleware([
    ...
    ->group(function () {
        ...
        Route::options('/user', function (Request $request) {
            return response()->noContent();
        })
            ->where('any', '.*')
            ->name('preflight');

        Route::prefix('resource-hub')
            ->name('resource-hub.')
            ->group(function () {
                Route::middleware([
                ...
                // This route MUST remain at /portals/... in order to catch requests to asset files and return the correct headers
                // NGINX has been configured to route all requests for assets under /portals to the application
                Route::get('assets/{file?}', [ResourceHubPortalController::class, 'asset'])
                    ->where('file', '(.*)')
                    ->name('asset');
            });
    });
