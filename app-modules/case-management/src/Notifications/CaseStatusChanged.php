...
class CaseStatusChanged extends BaseNotification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if (! $template) {
            return MailMessage::make()
                ...
                ->subject("Case {$this->case->case_number} status changed to {$this->case->status?->name}")
                ->line("The Case {$this->case->case_number} status has changed to {$this->case->status?->name}.")
    ...
    public function toDatabase(object $notifiable): array
    {
        return Notification::make()
            ->success()
            ->title((string) str("[Case {$this->case->case_number}](" . CaseResource::getUrl('view', ['record' => $this->case]) . ") status changed to {$this->case->status?->name}")->markdown())
            ->getDatabaseMessage();
    }
