...
it('executes alert workflow step successfully', function () {
    ...
    $alertDetails = WorkflowProactiveAlertDetails::factory()->create();

    $workflowRunStep = WorkflowRunStep::factory()->withDetails($alertDetails)->create([
        'workflow_run_id' => $workflowRun->id,
        'execute_at' => now(),
    ]);

    $job = new ProactiveAlertWorkflowActionJob($workflowRunStep);
    $job->handle();
    ...
    expect($alertDetails->description)->toBe($alert->description);
    expect($alertDetails->severity)->toBe($alert->severity);
    expect($alertDetails->status_id)->toBe($alert->status_id);
    expect($alertDetails->suggested_intervention)->toBe($alert->suggested_intervention);
