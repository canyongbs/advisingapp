...
<x-filament-panels::page full-height="true">
    <div class="flex h-full flex-col">
        <div class="grid flex-1 grid-cols-1 gap-6 md:grid-cols-4">
            <div class="col-span-1">
                <div class="flex flex-col gap-y-6">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        @if (count($conversations))
                        <ul class="flex flex-col gap-y-1 rounded-xl border border-gray-950/5 bg-white p-2 shadow-sm dark:border-white/10 dark:bg-gray-900">
                            @foreach ($conversations as $conversationItem)
                            @php
                            /** @var TwilioConversation $conversationItem */
                            @endphp
                            <li @class([ 'px-2 group cursor-pointer flex rounded-lg w-full items-center outline-none transition duration-75 hover:bg-gray-100 focus:bg-gray-100 dark:hover:bg-white/5 dark:focus:bg-white/5 space-x-1'
                                , 'bg-gray-100 dark:bg-white/5'=>
                                $conversation?->getKey() === $conversationItem->getKey(),
                                ])>
                                <button type="button" @class([ 'relative flex flex-1 items-center justify-between text-start gap-x-3 rounded-lg py-2 text-sm' , ])
            ...
            @endphp
            @if ($conversation)
            <div class="col-span-1 flex h-full flex-col gap-2 overflow-hidden md:col-span-3" x-data="userToUserChat({ selectedConversation: @js($conversation->getKey()), users: @js($users) })"
                ...
                <div class="col-span-1 flex h-full flex-col gap-2 overflow-hidden md:col-span-3" x-show="!loading && !error" x-transition.delay.850ms>
                    <div class="flex max-h-[calc(100vh-24rem)] flex-1 flex-col-reverse overflow-y-scroll rounded-xl border border-gray-950/5 text-sm shadow-sm dark:border-white/10 dark:bg-gray-800">
                            ...
                            <template x-for="message in messages" :key="message.message.index">
                                <div class="group w-full dark:bg-gray-800">
                                    <div class="m-auto justify-center px-6 py-3 text-base">
                                        <div class="mx-auto flex flex-1 gap-6 text-base md:max-w-2xl lg:max-w-[38rem] xl:max-w-3xl">
                                            <div class="relative mt-1 flex flex-shrink-0 flex-col items-end">
                                                <x-filament::avatar class="rounded-full" alt="User Avatar" x-bind:src="message.avatar" size="lg" />
                                            </div>
                                            <div class="relative flex w-[calc(100%-50px)] flex-col lg:w-[calc(100%-115px)]">
                                                <div class="flex max-w-full flex-grow flex-col gap-y-1">
                                                    <div class="flex items-center gap-2 text-sm">
                                                        <span class="font-medium text-gray-700 dark:text-gray-300" x-text="message.author"></span>
                                                        <span class="text-gray-500 dark:text-gray-500" x-text="formatDate(message.date)"></span>
                                                    </div>
                        ...
                        <template x-if="messagePaginator?.hasPrevPage">
                            <div class="mb-auto flex cursor-pointer justify-center bg-white p-3 text-center text-primary-500 dark:bg-gray-700" x-on:click="loadPreviousMessages">
                                <p>Load previous messages...</p>
                                <x-filament::loading-indicator class="ml-2 h-4 w-4 text-primary-500" x-show="loadingPreviousMessages" />
                            </div>
                        </template>
                    </div>
                    ...
                    <form @submit.prevent="submit">
                        <div class="w-full overflow-hidden rounded-xl border border-gray-950/5 bg-gray-50 shadow-sm dark:border-white/10 dark:bg-gray-700">
                            <div class="bg-white dark:bg-gray-800">
                                <div x-data="chatEditor({ currentUser: @js(auth()->id()), users: @js($users) })" x-model="message"
                                    x-on:click.outside="$refs.colorPicker.close(); $refs.emojiPicker.close()" wire:ignore x-modelable="content">
                                    <template x-if="isLoaded()">
                                        <div class="flex flex-wrap items-center gap-1 border-b px-3 py-2 dark:border-gray-700">
                                            <button class="rounded p-0.5" type="button" x-on:click="toggleBold()" x-bind:class="{ 'bg-gray-200 dark:bg-gray-700': isActive('bold', updatedAt) }">
                                                @svg('icon-bold', 'h-5 w-5')
                                            </button>

                                            <button class="rounded p-0.5" type="button" x-on:click="toggleItalic()" x-bind:class="{ 'bg-gray-200 dark:bg-gray-700': isActive('italic', updatedAt) }">
                                            ...
                                            <div>
                                                <button class="rounded p-0.5" type="button" x-on:click="toggleLink" x-bind:class="{ 'bg-gray-200 dark:bg-gray-700': isActive('link', updatedAt) }">
                                                ...
                                                <form
                                                    ...
                                                    x-on:submit.prevent="saveLink" x-cloak x-float.offset.placement.bottom-start="{ offset: 8 }" x-ref="linkEditor" x-on:click.outside="$el.close"
                                                    ...
                                                    <div class="flex flex-wrap items-center gap-3">
                                                        ...
                                                        <x-filament::button type="button" size="sm" x-show="linkUrl" x-on:click="removeLink" color="gray">
                                                            Remove
                                                        </x-filament::button>
                                                    </div>
                                                </form>
                                            ...
                                            <div>
                                                <button class="rounded p-0.5" type="button" x-on:click="$refs.colorPicker.toggle" x-bind:class="{
                                                ...
                                                <div class="absolute z-10 max-w-xs divide-y divide-gray-100 rounded-lg bg-white px-4 py-3 shadow-lg ring-1 ring-gray-950/5 transition dark:divide-white/5 dark:bg-gray-900 dark:ring-white/10"
                                                    x-cloak x-float.offset.placement.top-start="{ offset: 8 }" x-ref="colorPicker" x-transition:enter-start="opacity-0"
                                                    x-transition:leave-end="opacity-0">
                                                    <div class="flex flex-wrap items-center gap-2">
                                                        <button class="flex h-5 w-5 items-center rounded-full border bg-gray-50 dark:border-gray-400 dark:bg-gray-800" type="button"
                                                            x-on:click="removeColor()" x-bind:class="{
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#ef4444]" type="button" x-on:click="setColor('#ef4444')" x-bind:class="{
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#ec4899]" type="button" x-on:click="setColor('#ec4899')" x-bind:class="{
                                                                    'ring-2 ring-offset-2 ring-primary-600 dark:ring-offset-gray-900': updatedAt &&
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#3b82f6]" type="button" x-on:click="setColor('#3b82f6')" x-bind:class="{
                                                                    'ring-2 ring-offset-2 ring-primary-600 dark:ring-offset-gray-900': updatedAt &&
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#22c55e]" type="button" x-on:click="setColor('#22c55e')" x-bind:class="{
                                                                    'ring-2 ring-offset-2 ring-primary-600 dark:ring-offset-gray-900': updatedAt &&
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#eab308]" type="button" x-on:click="setColor('#eab308')" x-bind:class="{
                                                                    'ring-2 ring-offset-2 ring-primary-600 dark:ring-offset-gray-900': updatedAt &&
                                                        ...
                                                        <button class="h-5 w-5 rounded-full bg-[#737373]" type="button" x-on:click="setColor('#737373')" x-bind:class="{
                                                                    'ring-2 ring-offset-2 ring-primary-600 dark:ring-offset-gray-900': updatedAt &&
                                            ...
                                            <div>
                                                <button class="rounded p-0.5" type="button" x-on:click="$refs.emojiPicker.toggle">
                                                    @svg('heroicon-c-face-smile', 'mt-0.5 h-4 w-4')
                                                </button>

                                                <div class="absolute z-10 max-w-xs divide-y divide-gray-100 rounded-lg bg-white px-4 py-3 shadow-lg ring-1 ring-gray-950/5 transition dark:divide-white/5 dark:bg-gray-900 dark:ring-white/10"
                                                    x-cloak x-float.offset.placement.top-start="{ offset: 8 }" x-ref="emojiPicker" x-transition:enter-start="opacity-0"
                                                    x-transition:leave-end="opacity-0">
                                                    <div class="flex flex-wrap items-center gap-2">
                                                        <template
                                                            x-for="emoji in ['😀', '😂', '👍', '👎', '🙏', '😕', '🤔', '😊', '🎉', '💼', '🕒', '📅', '🔒', '❗', '❓', '💡', '🚫', '✅', '🤖', '📧', '🌐', '💬', '📈', '📉', '🤝']">
                                                            <button class="h-5 w-5 rounded hover:bg-gray-100 dark:hover:bg-gray-800" type="button" x-on:click="insertContent(emoji)" x-text="emoji"
                                                                x-bind:key="emoji"></button>
                                                        </template>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div class="flex items-center justify-between border-t px-3 py-2 dark:border-gray-600">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="relative flex h-6 items-center justify-center gap-0.5" x-show="usersTyping.length">
                                        <template x-for="user in usersTyping">
                                            <x-filament::avatar alt="User Avatar" size="w-4 h-4" x-bind:src="user.avatar" />
                                        </template>
                                        <span class="h-2 w-2 animate-bounce rounded-full bg-primary-500 text-primary-500 animation-delay-100"></span>
                                        <span class="h-2 w-2 animate-bounce rounded-full bg-primary-500 text-primary-500 animation-delay-200"></span>
                                        <span class="h-2 w-2 animate-bounce rounded-full bg-primary-500 text-primary-500 animation-delay-300"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <x-filament::modal id="confirmSafeLink" width="2xl">
        ...
        <x-slot name="description">
            <div x-data="{ href: null }" x-on:open-modal.window="href = $event.detail.href" class="flex flex-col mt-4">
                You are about to open another browser tab and visit:
                <strong x-text="href" class="mt-2"></strong>
            </div>
        </x-slot>

        <x-slot name="footerActions">
            <div x-data="{ href: null }" x-on:open-modal.window="href = $event.detail.href" class="flex justify-between w-full">
