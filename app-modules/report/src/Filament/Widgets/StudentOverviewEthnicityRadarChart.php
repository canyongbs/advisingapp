...
class StudentOverviewEthnicityRadarChart extends ChartReportWidget
{
    protected ?string $heading = 'Ethnicity';
    ...
    public function getData(): array
    {
        ...
        $ethnicityData = $shouldBypassCache
            ? $this->getEthnicityData($startDate, $endDate, $groupId)
            : Cache::tags(["{{$this->cacheTag}}"])->remember('student_by_ethnicity', now()->addHours(24), function () {
                return $this->getEthnicityData();
            });

        return [
            'labels' => $ethnicityData->pluck('ethnicity'),
            'datasets' => [
                [
                    'data' => $ethnicityData->pluck('count'),
                    'backgroundColor' => $ethnicityData->pluck('bg_color'),
                    'hoverOffset' => 4,
                ],
            ],
        ];
    ...
    protected function getEthnicityData(?Carbon $startDate = null, ?Carbon $endDate = null, ?string $groupId = null): Collection
    {
        return Student::query()
            ...
            ->groupBy('ethnicity_lower')
            ->orderBy('ethnicity_lower')
