...
class SisDataPipelineTable extends Component implements HasActions, HasForms, HasTable
{
    ...
    {
        return $table
            ->records(fn (): array => $this->fetchSisPipelineData())
            ->paginated(false)
            ->columns([
    ...
    {
        return view('student-data-model::livewire.sis-pipeline-data-table');
    }

    /**
     * @return array<int, array<string, mixed>>
     */
    public function fetchSisPipelineData(): array
    {
        try {
            ...
            if ($response->successful()) {
                ...
                foreach ($data as $index => $item) {
                    ...
                    $shortHash = substr($contentHash, 0, 8);
    ...
    {
        $totalItems = ($record['total_student'] ?? 0) +
                     ($record['total_enrollment'] ?? 0) +
                     ($record['total_program'] ?? 0);

        $processedItems = ($record['processed_students'] ?? 0) +
