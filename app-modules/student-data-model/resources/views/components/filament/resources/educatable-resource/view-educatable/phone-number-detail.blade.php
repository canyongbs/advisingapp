...
<button class="flex items-center gap-2" type="button" x-data="{ isLoading: false }"
    ...
    x-on:click="isLoading = true; $dispatch('send-sms', { phoneNumberKey: @js($phoneNumber->getKey()) })" @if (!$isOptedOut) x-tooltip.raw="Click to send an SMS" @endif @disabled(
        $isOptedOut ||
    ...
    @if ($isOptedOut)
        <x-filament::icon class="ml-1 h-6 w-6 text-danger-500" icon="heroicon-s-x-circle" x-tooltip.raw="SMS Opt Out" />
    @endif
</button>
