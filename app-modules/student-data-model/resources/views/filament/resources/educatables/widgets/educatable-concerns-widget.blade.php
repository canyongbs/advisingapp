...
<x-filament-widgets::widget>
    <x-filament::section class="h-full">
        <x-slot name="heading">Concerns</x-slot>

        <x-slot name="afterHeader">
            <x-filament::button color="gray" tag="a" :href="$this->getConcernsUrl()">Manage</x-filament::button>
        </x-slot>
        ...
        @if ($statusCounts = $this->getStatusCounts())
            <dl class="flex flex-wrap gap-3">
                @foreach ($statusCounts as $count)
                    @php
                        $filteredUrl = $this->getConcernsUrl(['status_id' => ['value' => $count['id']]]);
                    @endphp
