...
class CreateFormVersion
{
    /** @param array<string, mixed> $newData */
    public function execute(Form $oldVersion, array $newData): Form
    {
        return DB::transaction(function () use ($oldVersion, $newData) {
            ...
            $this->reassociateEmailAutoReply($oldVersion, $newVersion);

            return $newVersion;
        });
    }

    private function reassociateNotificationUsers(Form $oldVersion, Form $newVersion): void
    {
    ...
    private function reassociateWorkflowTriggers(Form $oldVersion, Form $newVersion): void
    {
    ...
    private function reassociateEmailAutoReply(Form $oldVersion, Form $newVersion): void
    {
        if ($oldVersion->emailAutoReply) {
            $oldVersion->emailAutoReply->form()->associate($newVersion);
            $oldVersion->emailAutoReply->save();
        }
    }
}
