...
test('assets endpoint points entry at the preview route only in preview mode', function () {
    ...
    $previewEntry = get(route('widgets.forms.api.assets', ['form' => $form, 'preview' => 'true']))
        ->assertSuccessful()
        ->json('entry');

    expect($previewEntry)->toContain("/api/forms/{$form->getKey()}/preview");

    $normalEntry = get(route('widgets.forms.api.assets', ['form' => $form]))
        ->assertSuccessful()
        ->json('entry');
