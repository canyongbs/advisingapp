...
test('no notification sent when users exist but no channel is enabled', function () {
    ...
    $form->notificationUsers()->attach($user);
    $submission = FormSubmission::factory()->create(['form_id' => $form->id]);

    (new SendFormNotificationJob($form, $submission))->handle();
...
test('care team members of a Student author are notified when notify_to_care_team is enabled', function () {
    ...
    Notification::assertSentTo($careTeamUser, FormSubmissionNotification::class);
});
...
test('care team is not notified when submission has no author', function () {
    Notification::fake();
...
test('a user appearing in all three recipient lists only receives one notification', function () {
    ...
    $form->notificationUsers()->attach($sharedUser);
    $submission = FormSubmission::factory()->create([
        'form_id' => $form->id,
        'author_type' => $student->getMorphClass(),
        'author_id' => $student->getKey(),
    ...
    Notification::assertSentTimes(FormSubmissionNotification::class, 1);
    Notification::assertSentTo($sharedUser, FormSubmissionNotification::class);
});
...
test('distinct users each receive their own notification without interference', function () {
    ...
    $form->notificationUsers()->attach($manualUser);
    $submission = FormSubmission::factory()->create([
        'form_id' => $form->id,
        'author_type' => $student->getMorphClass(),
        'author_id' => $student->getKey(),
    ...
    Notification::assertSentTimes(FormSubmissionNotification::class, 3);
    Notification::assertSentTo($manualUser, FormSubmissionNotification::class);
    Notification::assertSentTo($careTeamUser, FormSubmissionNotification::class);
    Notification::assertSentTo($subscriberUser, FormSubmissionNotification::class);
});
