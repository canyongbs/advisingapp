...
test('ViewResourceHubCategory is gated with proper access control', function () {
    ...
    actingAs($user)
        ->get(
            ResourceHubCategoryResource::getUrl('view', [
                'record' => $resourceHubCategory,
            ])
        )->assertForbidden();

    $user->givePermissionTo('resource_hub_category.view-any');
    $user->givePermissionTo('resource_hub_category.*.view');
