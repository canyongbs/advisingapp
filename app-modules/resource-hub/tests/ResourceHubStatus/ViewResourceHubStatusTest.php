...
test('ViewResourceHubStatus is gated with proper access control', function () {
    ...
    actingAs($user)
        ->get(
            ResourceHubStatusResource::getUrl('view', [
                'record' => $resourceHubStatus,
            ])
        )->assertForbidden();

    $user->givePermissionTo('resource_hub_status.view-any');
    $user->givePermissionTo('resource_hub_status.*.view');
