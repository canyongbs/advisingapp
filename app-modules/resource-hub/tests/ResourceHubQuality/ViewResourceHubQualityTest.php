...
test('ViewResourceHubQuality is gated with proper access control', function () {
    ...
    actingAs($user)
        ->get(
            ResourceHubQualityResource::getUrl('view', [
                'record' => $resourceHubQuality,
            ])
        )->assertForbidden();

    $user->givePermissionTo('resource_hub_quality.view-any');
    $user->givePermissionTo('resource_hub_quality.*.view');
