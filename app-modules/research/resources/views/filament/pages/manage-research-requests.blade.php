...
<div class="h-[calc(100dvh-4rem)]">
    @if ($this->isConsented)
        ...
        <div class="flex select-none flex-col gap-y-2">
            ...
            <template x-if="$wire.requestsWithoutAFolder.length">
                <ul class="flex flex-col gap-y-1 rounded-xl border border-gray-950/5 bg-white p-2 shadow-sm dark:border-white/10 dark:bg-gray-900"
                    id="folder-{{ null }}" x-on:drop.prevent="drop('{{ null }}')" x-on:dragenter.prevent
                    x-on:dragover.prevent>
                    <template x-for="request in $wire.requestsWithoutAFolder" :key="request.id">
                        <li :id="`request-${request.id}`" :class="{
                            ...
                            <div class="flex min-w-0 flex-1 items-center gap-3">
                                <template x-if="$wire.folders.length">
                                    <button type="button" draggable="true" x-on:dragstart="start(request.id, null)"
                                        x-on:dragend="end" :class="{
                                ...
                                <button
                                    ...
                                    type="button" x-on:click="selectRequest(request)">
                                    <span x-text="request.title" :class="{
                            ...
                            <div class="flex items-center gap-1">
                                <template x-if="loading.type !== 'moveRequestAction' || loading.identifier !== request.id">
                                    <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                        icon="heroicon-m-arrow-down-on-square" x-on:click="moveRequest(request.id)"
                                        label="Move request to a different folder" color="warning"
                                ...
                                <template x-if="loading.type !== 'editRequestAction' || loading.identifier !== request.id">
                                    <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                        icon="heroicon-m-pencil" x-on:click="editRequest(request.id)"
                                        label="Edit name of the request" color="warning"
                                        size="{{ ActionSize::ExtraSmall }}" />
                                </template>
                                <template x-if="loading.type === 'editRequestAction' && loading.identifier === request.id">
                                    <x-filament::loading-indicator
                                ...
                                <template
                                    ...
                                    <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                        icon="heroicon-m-trash" x-on:click="deleteRequest(request.id)"
                                        label="Delete the request" color="danger" size="{{ ActionSize::ExtraSmall }}" />
                                </template>
                                <template
                                    x-if="loading.type === 'deleteRequestAction' && loading.identifier === request.id">
                                    <x-filament::loading-indicator
            ...
            <template x-if="!$wire.requestsWithoutAFolder.length">
                <div class="flex flex-col gap-y-1 rounded-xl border border-dashed border-gray-950/5 bg-white px-3 py-2 text-gray-500 shadow-sm dark:border-white/10 dark:bg-gray-900"
                    x-show="dragging" x-on:drop.prevent="drop('{{ null }}')" x-on:dragenter.prevent x-on:dragover.prevent>
                    <div class="text-sm">
            ...
            <template x-if="$wire.folders.length">
                <div
                    ...
                    <template x-for="folder in $wire.folders" :key="folder.id">
                        <ul class="flex flex-col gap-y-1" :id="`folder-${folder.id}`" x-on:drop.prevent="drop(folder.id)"
                            x-on:dragenter.prevent x-on:dragover.prevent>
                            <span
                                ...
                                <x-filament::icon-button class="flex-shrink-0" icon="heroicon-o-folder-open"
                                    x-show="expanded(folder.id)" x-on:click="expand(folder.id)" />
                                <x-filament::icon-button class="flex-shrink-0" icon="heroicon-o-folder"
                                    x-show="! expanded(folder.id)" x-on:click="expand(folder.id)" />
                                ...
                                <div
                                    ...
                                    <div class="relative flex min-w-0 flex-1 items-center justify-center gap-x-3 rounded-lg py-2 text-sm"
                                        x-on:click="expand(folder.id)">
                                        <div class="min-w-0 flex-1 truncate" x-bind:class="{
                                    ...
                                    <div class="flex flex-shrink-0 items-center gap-1">
                                        <template
                                            ...
                                            <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                                icon="heroicon-m-pencil" x-on:click="renameFolder(folder.id)"
                                                label="Rename Folder" color="warning" size="{{ ActionSize::ExtraSmall }}" />
                                        </template>
                                        <template
                                            x-if="loading.type === 'renameFolderAction' && loading.identifier === folder.id">
                                            <x-filament::loading-indicator
                                        ...
                                        <template
                                            ...
                                            <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                                icon="heroicon-m-trash" x-on:click="deleteFolder(folder.id)"
                                                label="Delete Folder" color="danger" size="{{ ActionSize::ExtraSmall }}" />
                                        </template>
                                        <template
                                            x-if="loading.type === 'deleteFolderAction' && loading.identifier === folder.id">
                                            <x-filament::loading-indicator
                            ...
                            <template x-for="request in folder.requests" :key="request.id">
                                <li :id="`request-${request.id}`" x-show="expanded(folder.id)" :class="{
                                                'px-2 group flex rounded-lg w-full items-center outline-none transition duration-75 hover:bg-gray-100 focus:bg-gray-100 dark:hover:bg-white/5 dark:focus:bg-white/5 space-x-1': true,
                                                'bg-gray-100 dark:bg-white/5': request.id === $wire.selectedRequestId
                                            }">
                                    <div class="flex min-w-0 flex-1 items-center gap-3">
                                        <button type="button" draggable="true" x-on:dragstart="start(request.id, folder.id)"
                                            x-on:dragend="end" :class="{
                                                        'flex items-center cursor-move': true,
                                                        'text-gray-700 dark:text-gray-200': request.id !== $wire
                                                            .selectedRequestId,
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        <template
                                            ...
                                            <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                                icon="heroicon-m-pencil" x-on:click="editRequest(request.id)"
                                                label="Edit title of the request" color="warning"
                                                size="{{ ActionSize::ExtraSmall }}" />
                                        </template>
                                        <template
                                            x-if="loading.type === 'editRequestAction' && loading.identifier === request.id">
                                        ...
                                        <template
                                            ...
                                            <x-filament::icon-button class="relative hidden h-5 w-5 group-hover:inline-flex"
                                                icon="heroicon-m-trash" x-on:click="deleteRequest(request.id)"
                                                label="Delete the request" color="danger"
                                                size="{{ ActionSize::ExtraSmall }}" />
                                        </template>
                                        <template
                                            x-if="loading.type === 'deleteRequestAction' && loading.identifier === request.id">
        ...
        <div class="grid h-full flex-1 grid-cols-1 grid-rows-[1fr_auto] gap-2 lg:grid-cols-3 lg:gap-x-6 lg:gap-y-4 2xl:grid-cols-4"
            ...
            @if ($this->request)
                <div class="col-span-1 flex flex-col gap-2 overflow-y-auto pt-3 lg:col-span-2 lg:pt-6 2xl:col-span-3"
                    ...
                    <div class="mb-6 flex items-center justify-between gap-3">
                        ...
                        <div class="lg:hidden">
                            <x-filament::dropdown shift placement="bottom-start" width="lg"
            ...
            <div class="col-span-full mb-2 text-center text-xs md:hidden">
                © 2016-{{ date('Y') }} <a class="text-blue-600 underline dark:text-blue-400"
                    href="https://canyongbs.com/">Canyon GBS LLC</a>.
            </div>
        </div>
    @elseif (!$this->isConsented)
        <div x-init="$nextTick(() => $dispatch('open-modal', { id: 'consent-agreement' }))">
            <x-filament::modal id="consent-agreement" width="5xl" alignment="left" :close-by-clicking-away="false"
                :close-button="false">
                <x-slot name="header">
                ...
                <x-slot name="footer">
                    <form class="flex w-full flex-col gap-6" wire:submit="confirmConsent">
                        ...
                        <div class="flex justify-start gap-3">
                            <x-filament::button wire:click="denyConsent" outlined>
                                Cancel
                            </x-filament::button>
