...
return new class () extends SettingsMigration {
    public function up(): void
    {
        DB::transaction(function () {
            ...
            try {
                $this->migrator->add('ai.is_open_ai_gpt_41_mini_responses_api_enabled', false);
            } catch (SettingAlreadyExists $exception) {
                // do nothing
            }

            try {
                $this->migrator->add('ai.is_open_ai_gpt_41_nano_responses_api_enabled', false);
            } catch (SettingAlreadyExists $exception) {
                // do nothing
            }

            try {
                $this->migrator->add('ai.is_open_ai_gpt_o4_mini_responses_api_enabled', false);
            } catch (SettingAlreadyExists $exception) {
                // do nothing
            }
        });
