...
it('is scheduled to dispatch UpdateCurrentCustomerAdvisorLinks monthly on the first day at midnight', function () {
    ...
    $events = (new Collection($schedule->events()))->filter(function (mixed $event) use ($tenant) {
        return $event instanceof CallbackEvent
            && $event->getSummaryForDisplay() === "Dispatch UpdateCurrentCustomerAdvisorLinks | Tenant {$tenant->domain}"
...
it('dispatches UpdateCurrentCustomerAdvisorLinks when its scheduled callback runs', function () {
    Bus::fake();
    ...
    Bus::assertDispatched(UpdateCurrentCustomerAdvisorLinks::class);
});
