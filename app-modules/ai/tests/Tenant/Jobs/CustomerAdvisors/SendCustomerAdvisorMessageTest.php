...
it('surfaces the rate limit reset time and does not persist a partial response when the stream is rate limited', function () {
    ...
    dispatch_sync(new SendCustomerAdvisorMessage($advisor, $thread, 'Hello'));
    ...
    expect(CustomerAdvisorMessage::query()->where('is_advisor', true)->count())->toBe(0);

    Event::assertDispatched(
        CustomerAdvisorMessageChunk::class,
        fn (CustomerAdvisorMessageChunk $event): bool => $event->thread->is($thread)
...
it('completes and persists the response when the stream finishes normally', function () {
    ...
    Event::assertDispatched(
        CustomerAdvisorMessageChunk::class,
        fn (CustomerAdvisorMessageChunk $event): bool => $event->isComplete === true,
    );
});
