                ...
                <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-300">
                        ...
                        @foreach($getState() as $value => $change)
                            <tr>
                                <td class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $value }}</td>
                                <td class="px-3 py-4 text-sm text-gray-500">{{ isset($change['old']) ? is_array($change['old']) ? json_encode($change['old']) : $change['old'] : '' }}</td>
                                <td class="px-3 py-4 text-sm text-gray-500"><x-heroicon-m-chevron-right/></td>
                                <td class="px-3 py-4 text-sm text-gray-500">{{ isset($change['new']) ? is_array($change['new']) ? json_encode($change['new']) : $change['new'] : '' }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
