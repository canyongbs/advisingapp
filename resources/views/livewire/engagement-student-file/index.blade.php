    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="EngagementStudentFile" format="csv" />
                <livewire:excel-export model="EngagementStudentFile" format="xlsx" />
                <livewire:excel-export model="EngagementStudentFile" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($engagementStudentFiles as $engagementStudentFile)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $engagementStudentFile->id }}" wire:model="selected">
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('engagement_student_file_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.engagement-student-files.show', $engagementStudentFile) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('engagement_student_file_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.engagement-student-files.edit', $engagementStudentFile) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('engagement_student_file_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $engagementStudentFile->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
