    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="RecordProgramItem" format="csv" />
                <livewire:excel-export model="RecordProgramItem" format="xlsx" />
                <livewire:excel-export model="RecordProgramItem" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($recordProgramItems as $recordProgramItem)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $recordProgramItem->id }}" wire:model="selected">
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('record_program_item_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.record-program-items.show', $recordProgramItem) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('record_program_item_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.record-program-items.edit', $recordProgramItem) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('record_program_item_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $recordProgramItem->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
