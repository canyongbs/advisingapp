    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="CaseUpdateItem" format="csv" />
                <livewire:excel-export model="CaseUpdateItem" format="xlsx" />
                <livewire:excel-export model="CaseUpdateItem" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($caseUpdateItems as $caseUpdateItem)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $caseUpdateItem->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $caseUpdateItem->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('case_update_item_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.case-update-items.show', $caseUpdateItem) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('case_update_item_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.case-update-items.edit', $caseUpdateItem) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('case_update_item_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $caseUpdateItem->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
