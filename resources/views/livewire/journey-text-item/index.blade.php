    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="JourneyTextItem" format="csv" />
                <livewire:excel-export model="JourneyTextItem" format="xlsx" />
                <livewire:excel-export model="JourneyTextItem" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($journeyTextItems as $journeyTextItem)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $journeyTextItem->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $journeyTextItem->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('journey_text_item_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.journey-text-items.show', $journeyTextItem) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('journey_text_item_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.journey-text-items.edit', $journeyTextItem) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('journey_text_item_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $journeyTextItem->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
