    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="KbItem" format="csv" />
                <livewire:excel-export model="KbItem" format="xlsx" />
                <livewire:excel-export model="KbItem" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($kbItems as $kbItem)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $kbItem->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $kbItem->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('kb_item_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.kb-items.show', $kbItem) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('kb_item_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.kb-items.edit', $kbItem) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('kb_item_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $kbItem->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
